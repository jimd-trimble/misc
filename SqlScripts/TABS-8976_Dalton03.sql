...
declare @displayLicId bigint = 58;
declare @canAutoSyncId bigint = 7;
...
select o.ID as OrgID, o.Name, lic.LicenseLevel, fcnt.FieldCount, dcnt.DeviceCount, devNames.DeviceNames, displayLic.DisplayLicenseCount, displayLic.AutoSyncEnabledCount
	, logins.LoginCount, UserLicense.licCount as UserLicenseCount
...
-- Display Licenses and AutoSyncEnabled:
left outer join (
	select ad.fk_OrgID, count(distinct l.ID) as DisplayLicenseCount, count(distinct sync.fk_DeviceID) as AutoSyncEnabledCount from AssetDevice ad
	join @variables v on ad.fk_orgid = v.orgid
	join Device d on ad.fk_DeviceID = d.ID
	join DeviceType dt on d.fk_DeviceTypeID = dt.ID
	join DeviceLicenseUsage dlu on d.ID = dlu.fk_DeviceId
	join License l on dlu.fk_LicenseID = l.ID
	join LicenseType lt on l.fk_LicenseTypeID = lt.ID
	left outer join (
		select fk_DeviceID from DeviceSyncSettings dss
		where dss.fk_AutoSyncStatusID = @canAutoSyncId
	) sync on d.ID = sync.fk_DeviceID
	where ad.IsCurrent = 1
	and l.fk_LicenseTypeID = @displayLicId
	group by ad.fk_OrgID
) displayLic on o.ID = displayLic.fk_OrgID
...
left outer join (
	...
	group by fk_orgid
) logins on o.ID = logins.fk_orgid
